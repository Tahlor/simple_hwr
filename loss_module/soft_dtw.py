...
from loss_module.losses import CustomLoss
import logging
...
class SoftDTW(CustomLoss):
    def __init__(self, loss_indices, gamma=1.0, normalize=False, swap_inputs=False, **kwargs):
        self.__dict__.update(**kwargs) # do this first, may be overwritten by call to CustomLoss
        super(SoftDTW, self).__init__(loss_indices, **kwargs)
    ...
    def lossfun(self, preds, targs, label_lengths, *args, **kwargs):
        loss = 0
        for i in range(len(preds)):  # loop through BATCH
            loss += (self.forward(preds[i][:, self.loss_indices], targs[i][:, self.loss_indices]) * self.subcoef).sum()
