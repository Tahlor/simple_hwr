...
from trainers import AlexGravesTrainer, TrainerStrokeRecovery
from hwr_utils.stroke_dataset import BasicDataset
from hwr_utils.stroke_recovery import *
from hwr_utils import utils
from torch.optim import lr_scheduler
from models.stroke_model import *
...
def main(config_path):
    ...
    config_path = "/media/data/GitHub/simple_hwr/RESULTS/OFFLINE_PREDS/AG/RESUME.yaml"
    ...
    load_path_override = "/media/data/GitHub/simple_hwr/RESULTS/OFFLINE_PREDS/AG/AlexGravesCombined_super_images_only_model.pt"
    ...
    for load_path_override in [load_path_override
        ...
        config = train_stroke_recovery.main(config_path, eval_only=True, load_path_override=_load_path_override)
        ...
        model.eval()
        eval_only(eval_loader, model, trainer)
...
def eval_only(dataloader, model, trainer):
    distances = []
    final_out = []
    ...
    for i, item in enumerate(tqdm(dataloader)):
        preds = trainer.generate(item)
        #print(preds[0, 0:5, 0:2])
        preds[:,:,0] = np.cumsum(preds[:,:,0], axis=1) # unrelativefy
        #print(preds[0,0:5, 0:2])
